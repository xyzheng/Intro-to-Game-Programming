...
bool collided(Entity* e, Entity* e2){
	//do the multiplication for the four points for each entity
	Vector a1 = e->matrix * e2->topLeft;
	a1 = e2->inverse * a1;
	Vector b1 = e->matrix * e2->topRight;
	b1 = e2->inverse * b1;
	Vector c1 = e->matrix * e2->bottomLeft;
	c1 = e2->inverse * c1;
	Vector d1 = e->matrix * e2->bottomRight;
	d1 = e2->inverse * d1;

	Vector a2 = e->matrix * e2->topLeft;
	a2 = e->inverse * a2;
	Vector b2 = e->matrix * e2->topRight;
	b2 = e->inverse * b2;
	Vector c2 = e->matrix * e2->bottomLeft;
	c2 = e->inverse * c2;
	Vector d2 = e->matrix * e2->bottomRight;
	d2 = e->inverse * d2; 
	...
	//collide
	if (eSmallX <= e2->right && eLargeX >= e2->left && eSmallY <= e2->top && eLargeY >= e2->bot && e2SmallX <= e->right && e2LargeX >= e->left && e2SmallY <= e->top && e2LargeY >= e->bot){
		if(e->x < e2->x){
			e->x -= 0.01f;
			if (e->y < e2->y){
				e->y -= 0.01f;
