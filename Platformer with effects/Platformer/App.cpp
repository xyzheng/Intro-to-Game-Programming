...
App::App() {
	...
	emitter = new ParticleEmitter(500);
	perlinValue = 0.0f;
...
void App::updateGameLevel(){
	...
	animationTime = animationTime + FIXED_TIMESTEP;
	...
	screenShakeIntensity = easeIn(0.4f, 0.0f, animationTime);
	//jump
	if(state == STATE_GAME_LEVEL && keys[SDL_SCANCODE_SPACE] && player.collidedBot == true){
		player.yVelo = 3.0f;
		player.collidedBot = false;
	...
	emitter->position.x = player.x;
    emitter->position.y = player.y;
